# valcore — CLI, `~/.valcore`, and Homebrew distribution

**Date:** 2026-07-30
**Status:** Approved, ready for implementation planning
**Builds on:** `2026-07-26-valcore-design.md`

## Purpose

Turn valcore from a repo you run with `uvicorn` into a tool you install:
...
`~/.valcore`, and a Homebrew formula.
...
- **`api/` is not in the wheel.** `[tool.hatch.build.targets.wheel] packages = ["src/valcore"]`
...
- **PyPI name `valcore` is available** (`valcore` is taken, but that is only a distribution
  name; our import package `valcore` is unaffected).

## Part 1 — Making the project installable

### `api/` moves to `src/valcore/api/`
...
Mechanical consequences: every `from api.x import y` becomes `from valcore.api.x import y`; the
...
`valcore.api.routes.<name>`.

### The SPA ships as package data

`web/dist` is built at release time and force-included into the wheel as
`src/valcore/web_dist/`. It stays gitignored — build output does not belong in git.
...
`valcore.db`, `valcore.db-shm`, `valcore.db-wal` are added to `.gitignore` — they are currently
...
Entry point: `[project.scripts] valcore = "valcore.cli:main"`.

```
valcore serve [--port 8000] [--host 127.0.0.1] [--no-browser]
valcore run <evaluator> <dataset> [--version NAME] [--kind validation|eval]
                                    [--concurrency 8] [--json] [--watch]
                                    [--min-accuracy FLOAT]
valcore list evaluators | datasets | runs [--json]
valcore export <evaluator> [--version NAME] [-o out.py]
valcore config set-key | get | path | edit
valcore version
```
...
is the hook that lets valcore gate a pipeline.
...
Location is overridable with `VALCORE_HOME`, which exists primarily so tests and CI never touch
a real home directory. The directory is created on first use.

### Settings precedence

```
CLI flag  >  VALCORE_* env var  >  ~/.valcore/config.toml  >  built-in default
```

This extends the existing `Settings` (pydantic-settings, `VALCORE_` prefix) with a TOML source
placed below env vars. `db_path` default becomes `~/.valcore/valcore.db`.
...
There is no meaningful installed base, so no migration runs. If `./valcore.db` exists in the
working directory and `~/.valcore/valcore.db` does not, the CLI prints a one-line notice
...
`packaging/valcore.sh` provisions a venv from PyPI **wheels** on first run — no compilation, so
...
| Area | Approach |
|---|---|
| CLI | `click.testing.CliRunner` with `VALCORE_HOME` pointed at `tmp_path`; every command's happy path, `--json` shape, and error exits |
...
| Packaging | CI builds the wheel and asserts `valcore/api/` and `valcore/web_dist/index.html` are present and the `valcore` entry point resolves |
| Launcher | `shellcheck packaging/valcore.sh`, plus a test running it with a stub `uv` on `PATH` asserting venv creation and the stamp-triggered re-provision |

No test may touch a real `~/.valcore` or reach the network.
