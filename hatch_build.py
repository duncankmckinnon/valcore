...
wheel as ``evalcore/web_dist``. Declaring the force-include statically in
...
class SpaDistBuildHook(BuildHookInterface):
    """Ship ``web/dist`` as ``evalcore/web_dist`` when the SPA build output exists."""

    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        dist_dir = Path(self.root, "web", "dist")
        if dist_dir.is_dir():
            build_data.setdefault("force_include", {})[str(dist_dir)] = "src/evalcore/web_dist"
